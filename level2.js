...
export default function() {
  ...
  const exercise3 = _ => {
    return reduce((acc, x) => x.eyeColor === 'green' ? acc + 1: acc, 0, clients)
  }
  ...
  const exercise5 = _ => {
    return compose(
map(x => x.address.street + ' ' + x.address.city + ', ' + x.address.state + ' ' + x.address.zip),
filter(result => result.address.state === 'North Carolina')
)(clients)
  }
  ...
  const exercise7 = _ => {
    return reduce((acc, x) => x.age >= 22 & x.age <= 27 && x.eyeColor === 'green' && x.gender === 'male' ? acc + 1 : acc, 0, clients)
  }
